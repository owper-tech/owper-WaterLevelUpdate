...
#Paths
path="//Fldep1/OWPER/EVG/Programmatic/WaterQuality/R_Reports/RegSch"
data.path=paste(path,"/Data/",sep="")
...
#Functions
reg.sch.lines.fun=function(data.path,WY=CurWY){
...
kcol=reg.sch.lines.fun(paste(data.path,"KCOL_RegSch3.csv",sep=""))
...
#Lake Okeechobee
LORS=reg.sch.lines.fun(paste(data.path,"LORS.csv",sep=""))
...
#WMA
#Roten
roten=reg.sch.lines.fun(paste(data.path,"Roten_2010InterimOps.csv",sep=""))
...
#Holey
holey=reg.sch.lines.fun(paste(data.path,"Holey.csv",sep=""))
...
#WCA1
wca1=reg.sch.lines.fun(paste(data.path,"WCA1.csv",sep=""))
...
#WCA2
wca2=reg.sch.lines.fun(paste(data.path,"WCA2.csv",sep=""))
...
wca3=reg.sch.lines.fun(paste(data.path,"WCA3A_Schedule2.csv",sep=""))
...
<center>![KCOL](KCOL2_v2.jpg)</center>
...
<center>![LakeO](LakeO3.jpg)</center>
...
<center>![WMA](RotenHoley_1.jpg)</center>
...
<ul><font size=2>
<li>01 Jan  - 30 Jun use 1-8C gauge. </li>
<li>01 Jul - 31 Dec use 1-8C gauge, unless rising stage when canal (1-8C) exceeds mean stage (1-7, 1-8T & 1-9) then use mean stage.  </li>
</font></ul>
...
<ul><font size=2>
<li>01 Jan  - 31 Jan use 2-17 gauge, unless 2-17 recedes to 11.5ft then use S-11B. </li>
<li>31 Jan - 30 Jun use S-11B gauge. </li>
<li>01 Jul - 31 Dec use 2-17 gauge.</li>
</font></ul>
...
<center>![WCA3](WCA3_v2_2.jpg)</center>
...
srs.flow.dbkey=data.frame(SITE=c(paste0("S12",LETTERS[1:4]),"S333","S333N","S334","S356"),DBKEY=c("01313","00610","00621","01310","91487","40371","91488","64136"))
...
srs.q.da.xtab$S333R=with(srs.q.da.xtab,ifelse((S333+S333N)-S334<0,0,(S333+S333N)-S334))
...
ylim.val=c(0,round(max(ceiling(srs.q.da.xtab$TFlow))+max(ceiling(srs.q.da.xtab$TFlow))*0.1,-2));by.y=1000;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
...
legend.txt=c("S12A","S12B","S12C","S12D","(S333+S333N)-S334","S356");pch.vals=22
...
plot(TFlow~Date.EST,srs.q.da.xtab,xlim=xlim.val,ylim=ylim.val,yaxt="n",xaxt="n",ylab=NA,xlab=NA,yaxs="i",type="n",axes=F)
abline(h=ymaj,v=xmaj,lty=3,col="grey")
with(srs.q.da.xtab,shaded.range(Date.EST,rep(0,nrow(srs.q.da.xtab)),(S333+S333N),cols[5],col.adj=col.adj.val,lty=1))
axis_fun(2,ymaj,ymin,ymaj,1)
axis_fun(1,xmaj,xmin,format(xmaj,"%m-%Y"),1);box(lwd=1)
...
leg.x=0.5;#xlim.val[1]+(diff(xlim.val)/2);
leg.y=0.5;#ylim.val[1]-7*1000
legend.txt=c("S333+S333N","S333 HW","9.2 Ft Stage");
...
legend(xleg,yleg,
       legend=c("Mean L29 Stage","L-29 Limit (7.5 Feet)","FDOT Maximum Stage Constraint"),lwd=c(2,2,2,2),col=c("dodgerblue","red","gray50"),
       lty=c(1,1,2),pt.cex=3,ncol=2,cex=1,bty="n",y.intersp=1,x.intersp=0.75,xpd=NA,xjust=0.5)

ylim.val=c(2,10);by.y=2;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/2)
plot(mean.val~DATE,L29,type="n",xlim=xlim.vals,ylim=ylim.val,ylab=NA,xlab=NA,yaxs="i",xaxs="i",xaxt="n",yaxt="n")
abline(h=ymaj,v=xmaj,lty=3,col="grey")
abline(v=date.fun(paste(CurWY-1,05,01,sep="-")),lwd=2)
...
<center>![CSSS](CSSS-ENP2.jpg)</center>
...
<center>![FDEP](deplogo.png)</center>

<font size=6 face="Times New Roman, Times, serif"><center>Office of Ecosystem Projects</center></font>